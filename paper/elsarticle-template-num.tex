...
All code, data generation pipelines, prompting scripts, and analysis tools used in this study are publicly available at:

\begin{center}
\url{https://github.com/GSoli96/LLM-Pain-Assessment-MPQ}
\end{center}

The repository includes: (i)~the synthetic dataset generation pipeline (\dbsuite); (ii)~the LLM querying script with all 13 prompting strategy--task combinations; (iii)~the metric analysis script (\texttt{analisi\_cloud\_results.py}); and (iv)~configuration templates for API key management (\texttt{ChaviOllama.json.example}). The four synthetic datasets (500 records each, generated by ChatGPT-4, Claude~3.7, DeepSeek~R1, and DoctorAI) are described in Section~\ref{sec:dataset_gen}. Raw result CSVs are excluded from version control due to size ($\approx$\,64\,MB each) but can be regenerated by running the provided pipeline. All models are accessed via the Ollama Cloud API with a fixed decoding temperature of~0 to ensure deterministic and reproducible outputs.
